...
bool checkCoordinates(AnalyzeLidarData ald, FieldProperties fP, String text, double x, double y, String input) {
  ...
  ResultOrError<LidarInfos> infos = ald.getLidarInfos();
  SerialDebug.println("erreur ? : ");
  SerialDebug.print(infos.hasError());
  if (infos.hasError()) {
    return false;
  }
  double tolerance = 8;  // cm
  Vector2 robotCoordinates = infos.value().coordinates();
...
void setup() {
  ...
  log_a(ErrorLevel, "setup", "setup complete");
  log_a(ErrorLevel, "setup", "free ram : " + String(freeram()));
}

void loop() {
  log_a(ErrorLevel, "loop", "start loop");
